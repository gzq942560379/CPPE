#!/bin/bash


cd ./build/ && make && cd ..

echo 00 --------------------------------------------------
./build/llvmassignment test-bc/test00.bc
echo 01 --------------------------------------------------
./build/llvmassignment test-bc/test01.bc
echo 02 --------------------------------------------------
./build/llvmassignment test-bc/test02.bc
echo 03 --------------------------------------------------
./build/llvmassignment test-bc/test03.bc
echo 04 --------------------------------------------------
./build/llvmassignment test-bc/test04.bc
echo 05 --------------------------------------------------
./build/llvmassignment test-bc/test05.bc
echo 06 --------------------------------------------------
./build/llvmassignment test-bc/test06.bc
echo 07 --------------------------------------------------
./build/llvmassignment test-bc/test07.bc
echo 08 --------------------------------------------------
./build/llvmassignment test-bc/test08.bc
echo 09 --------------------------------------------------
./build/llvmassignment test-bc/test09.bc
echo 10 --------------------------------------------------
./build/llvmassignment test-bc/test10.bc
echo 11 --------------------------------------------------
./build/llvmassignment test-bc/test11.bc
echo 12 --------------------------------------------------
./build/llvmassignment test-bc/test12.bc
echo 13 --------------------------------------------------
./build/llvmassignment test-bc/test13.bc
echo 14 --------------------------------------------------
./build/llvmassignment test-bc/test14.bc
echo 15 --------------------------------------------------
./build/llvmassignment test-bc/test15.bc
echo 16 --------------------------------------------------
./build/llvmassignment test-bc/test16.bc
echo 17 --------------------------------------------------
./build/llvmassignment test-bc/test17.bc
echo 18 --------------------------------------------------
./build/llvmassignment test-bc/test18.bc
echo 19 --------------------------------------------------
./build/llvmassignment test-bc/test19.bc